...
$app->patch("/events/{uid}", function ($request, $response, $arguments) {

    /* Check if token has needed scope. */
    if (false === $this->token->hasScope(["event.all", "event.update"])) {
        throw new ForbiddenException("Token not allowed to update events.", 403);
    }

    /* Load existing event using provided uid */
    if (false === $event = $this->spot->mapper("App\Event")->first([
        "uid" => $arguments["uid"]
    ])) {
        throw new NotFoundException("Event not found.", 404);
...
$app->put("/events/{uid}", function ($request, $response, $arguments) {
...
$app->delete("/events/{uid}", function ($request, $response, $arguments) {

    /* Check if token has needed scope. */
    if (false === $this->token->hasScope(["event.all", "event.delete"])) {
