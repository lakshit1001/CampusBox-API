<?php
 ...
 $app->get("/notifications", function ($request, $response, $arguments) {
    ...
    $appreciate = $this->spot->mapper("App\ContentAppreciate")
        ->query("SELECT content_appreciates.content_id,contents.title, content_appreciates.timer, content_appreciates.username, COUNT(content_appreciates.content_id) AS x FROM `content_appreciates`
                LEFT JOIN `contents`
