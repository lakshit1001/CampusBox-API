...
class dbHelper {
    ...
    function selectToMax($table, $columns, $where, $from, $field, $order){
        try{
            $a = array();
            $w = "";
            foreach ($where as $key => $value) {
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table." where 1=1 ".$w." and ".$field." between ".$from." and "." (select MAX(".$field.") from ".$table." ) "." order by ".$order);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
    ...
    function selectRange($table, $columns, $where, $from, $to, $field, $order){
        try{
            $a = array();
            $w = "";
            foreach ($where as $key => $value) {
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table." where 1=1 ".$w." and ".$field." between ".$from." and ".$to." order by ".$order);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
    ...
    function selectjoin($table1,$table2, $col1,$col2, $where){
        try{
            $a = array();
            $w = "";
            foreach ($where as $key => $value) {
            ...
            $stmt = $this->db->prepare("select * from ".$table1." inner join ".$table2 ." on ".$table1.".".$col1."=".$table2.".".$col2. " having  1=1  ". $w);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
